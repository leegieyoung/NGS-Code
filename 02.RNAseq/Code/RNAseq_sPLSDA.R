...
#sample.group <- factor(sample.pheno, levels=c("NSNP","stricture","penetrating"))
...
#DIR
print("Make_DIR")
work_dir <- "/data/keeyoung/gy_RNA/iCD/test/"
...
write.table(y_uniq.lcpm, paste0(work_dir,"02.Table/CD_normal_rmUnknown.csv"), sep='\t', col.names=T, row.names=T, quote=F)
...
sample.col <- gsub("stricture","#F68B33",sample.col) #orange
sample.col <- gsub("penetrating","#C2C2C2",sample.col) #gray
sample.col <- gsub("NSNP","#388ECC",sample.col) #blue

#legend=list(legend=levels(sample.group),
legend=list(legend=c("stricture","penetrating","NSNP"),
