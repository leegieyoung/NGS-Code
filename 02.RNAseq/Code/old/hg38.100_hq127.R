...
#tx2gene <- read.delim("/data/keeyoung/REFERENCE/RNA/gtf/tx2gene.GRCh38.79.tab")
...
#tximport - Annotate to gene & Calculate - length,counts,abundance
sample.tx <- list()
...
#edgeR - DGElist -Convert data to list-based data, filterExpr - Only exist worthwhile Gene, calcNormFactors - Minimize logFC in samples, estimateDisp - estimate Dispersion
...
png(filename="/data/keeyoung/gy_RNA/03.Plot/hg38_127_PCA.png")
...
write.csv(CDvsUC_in_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_127_CDvsUC_in.csv", row.names=TRUE)
write.csv(CDvsUC_no_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_127_CDvsUC_no.csv", row.names=TRUE)
write.csv(INvsNO_CD_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_127_INvsNO_CD.csv", row.names=TRUE)
write.csv(INvsNO_UC_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_127_INvsNO_UC.csv", row.names=TRUE)
