...
#tximport
tx2gene <- read.delim("/data/keeyoung/REFERENCE/RNA/gtf/tx2gene.GRCh38.104.tab")
sample.ID <- scan("/data/keeyoung/gy_RNA/01.Sample/234.IBD.sample", what=character(0))
sample.pheno <- scan("/data/keeyoung/gy_RNA/01.Sample/234.IBD.pheno", what=character(0))
pca.pheno <- scan("/data/keeyoung/gy_RNA/01.Sample/234.IBD.pheno", what=character(0))
names(sample.pheno) <- sample.ID
salmon_root <- "/data/keeyoung/gy_RNA/02.hg38_salmon_result/"
files <- file.path(salmon_root, sample.ID, "rmVer_quant.sf")
...
sample.DGElist <- DGEList(cnt, group=sample.group, genes=sample.df.cnt[,0,drop=FALSE])
...
png(filename="/data/keeyoung/gy_RNA/03.Plot/hg38_234_PCA.png")
...
write.csv(CDvsUC_in_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_234_CDvsUC_in.csv", row.names=TRUE)
write.csv(CDvsUC_no_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_234_CDvsUC_no.csv", row.names=TRUE)
write.csv(INvsNO_CD_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_234_INvsNO_CD.csv", row.names=TRUE)
write.csv(INvsNO_UC_result, "/data/keeyoung/gy_RNA/04.CSV/hg38_234_INvsNO_UC.csv", row.names=TRUE)
