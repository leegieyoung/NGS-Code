...
Genes <- data.frame(t(y_uniq.lcpm[which(rownames(y_uniq.lcpm) %in% c("TNF","IL6","FAP")),]))
...
#OnlyRNR
png(filename=paste0(work_dir,"01.Plot/RNR_TNF_IL6.png"), width=60, height=50, units="cm", res=200)
ggplot(data=OnlyRNR_Genes, mapping=aes(x=logTNF,y=logIL6, shape=RNR, color=RNR)) + 
    geom_point(size=5) + 
    geom_smooth(method=lm, se=FALSE) + 
    xlab(expression("log"[2]*"TNF")) +
    ylab(expression("log"[2]*"IL6")) +
