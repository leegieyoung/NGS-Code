...
merge <- cbind(Total_CDno,Tru_CDno)
merge <- merge[,na.omit(match(sample.ID,colnames(merge)))]
sample.ID <- sample.ID[which(sample.ID %in% colnames(merge))]
sample.pheno <- sample.pheno[which(names(sample.pheno) %in% colnames(merge))]

#It is working when all.equal(sample.ID,colnames(merge)) is "TRUE".
if(all.equal(sample.ID,colnames(merge))=="TRUE"){
...
#platform
sample.meta$V1 <- gsub("-",".", sample.meta$V1)
sample.platform <- sample.meta$V4[which(sample.meta$V1 %in% sample.ID)]
names(sample.platform) <- sample.meta$V1[which(sample.meta$V1 %in% sample.ID)]
names(sample.platform) <- gsub("-",".",names(sample.platform))
length(sample.platform)
sample.platform <- sample.platform[which(names(sample.platform) %in% names(sample.pheno))]

if(all.equal(names(sample.platform), names(sample.pheno))=="TRUE"){
print({paste0("True_","Dim is ",length(sample.platform))})
} else {
print({"FALSE"})
print({head(names(sample.platform))})
print({head(names(sample.pheno))})
}
...
print({dim(iCD)})
#match
iCD <- iCD[,na.omit(match(sample.ID, colnames(iCD)))]
sample.ID <- sample.ID[which(sample.ID %in% colnames(iCD))]
sample.pheno <- sample.pheno[which(names(sample.pheno) %in% colnames(iCD))]

all.equal(sample.ID, colnames(iCD))
all.equal(names(sample.pheno), colnames(iCD))
##################
#Loading data
print("Loading data")
...
train.X <- merge[,which(colnames(merge) %in% colnames(Total_CDno))]
test.X <- merge[,which(colnames(merge) %in% colnames(Tru_CDno))]

train.Y <- sample.group[which(names(sample.group) %in% colnames(Total_CDno))]
test.Y <- sample.group[which(names(sample.group) %in% colnames(Tru_CDno))]

all.equal(colnames(train.X), names(train.Y))
all.equal(colnames(test.X), names(test.Y))
...
#perf.splsda.train <- readRDS(paste0(work_dir,"perf.splsda.train.rds"))
...
#tune.splsda.train <- readRDS(paste0(work_dir,"tune.splsda.train.rds"))
...
write.table(Predict_result, paste0(work_dir,"02.Table/Predict_result.txt"),sep='\t', quote=F)
...
png(filename=paste0(work_dir,"01.Plot/sPLSDA_perf_test.png"), width=60, height=50, units="cm", res=200)
print({plot(perf.splsda.test, col = color.mixo(5:7), sd = TRUE,
    legend.position = "horizontal")})
dev.off()
saveRDS(perf.splsda.test,paste0(work_dir,"perf.splsda.test.rds"))
...
tune.splsda.test <- tune.splsda(test.X, test.Y, ncomp = 3, # calculate for first 4 components
                                 ...
                                 cpus = 32) # allow for paralleliation to decrease runtime
#)
png(filename=paste0(work_dir,"01.Plot/sPLSDA_tune_test.png"), width=60, height=50, units="cm", res=200)
print({plot(tune.splsda.test, col = color.jet(3))})
dev.off()
saveRDS(tune.splsda.test,paste0(work_dir,"tune.splsda.test.rds"))
...
opti <- cbind(optimal.ncomp, optimal.keepX)
write.table(opti, paste0(work_dir,"optimal.test_keepX.txt"),sep='\t', quote=F)
...
test.col <- gsub("stricture","#F68B33",test.col) #orange
test.col <- gsub("penetrating","#C2C2C2",test.col) #gray

legend=list(legend=levels(test.Y),
            col=c("#388ECC","#F68B33","#C2C2C2"),
#            col=c("#F68B33","#C2C2C2", "#388ECC"),
            title="Last Behavior",
            cex=1.4)

png(filename=paste0(work_dir,"01.Plot/sPLSDA_test_final_heatmap_comp1.png"), width=70, height=30, units="cm", res=200)
...
write.table(test.gene, paste0(work_dir,"02.Table/test.gene.txt"),sep='\t', quote=F)
