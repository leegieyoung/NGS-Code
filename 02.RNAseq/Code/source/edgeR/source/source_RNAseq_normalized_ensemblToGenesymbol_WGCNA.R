...
names(moduleGenes) <- colnames(datExpr)
#
MEs_module <- MEs[,c("MEblue", "MEbrown")]
write.table(MEs_module, paste0(work_dir,"02.Table/",name,"modue.txt"), sep='\t', quote=F)
...
blueGenes <- as.data.frame(moduleGenes[which(moduleGenes %in% "blue")])
write.table(blueGenes, paste0(work_dir,"02.Table/",name,"_module_blue_Genes.txt"), quote=F, sep='\t')

brownGenes <- as.data.frame(moduleGenes[which(moduleGenes %in% "brown")])
write.table(brownGenes, paste0(work_dir,"02.Table/",name,"_module_brown_Genes.txt"), quote=F, sep='\t')
