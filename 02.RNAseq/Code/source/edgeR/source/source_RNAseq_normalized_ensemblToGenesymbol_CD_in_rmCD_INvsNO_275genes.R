...
Verified_Gene <- read.table("/data/keeyoung/gy_RNA/06.output/CD_in_Tru/02.Table/CD_in_Tru_qcFtest_rmINvsNO_table.txt",sep='\t')
print(dim(Verified_Gene))
Verified_Gene <- rownames(Verified_Gene)
y_uniq <- y_uniq[which(y_uniq$genes$gene_name %in% Verified_Gene),]
print(dim(y_uniq))
...
m <- match(qcFtest$gene$gene_name , egENTREZID$symbol)

qcFtest$genes$entrezID <- egENTREZID$gene_id[m]
David <- cbind(qcFtest$genes$entrezID ,qcFtest$table$logFC)
write.table(David,paste0(work_dir,"02.Table/",name,"_qcFtest_rmCD_INvsNO_table_David.txt"), quote=F, sep=',')
