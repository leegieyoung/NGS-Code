...
#Sample pheno
sample.pheno <- gsub("penetrating","others",sample.pheno)
sample.pheno <- gsub("stricture","others",sample.pheno)

if(all.equal(sample.ID, colnames(iCD))=="TRUE"){
sample.group <- factor(sample.pheno, levels=c("NSNP","others"), order = T)
sample.design <- model.matrix(~0 + sample.group)
colnames(sample.design) <- levels(sample.group)
}

my.contrasts <- makeContrasts(class=others-NSNP, levels=sample.design)
...
genes <- c("NPTX2","MUC6","NRAP","AL157935.3","WFDC2","POTEC","SCEL","CYP2D6","GLDN","NEFH","ST6GAL2","B3GNT7","ADAMTS4","THPO","CPA6","ST3GAL4","CLDN8")
valified_Ftest <- Ftest[which(rownames(Ftest) %in% genes),]
write.table(valified_Ftest$table,paste0(work_dir,"02.Table/",name,"_valified_Ftest_table.txt"), quote=F, sep='\t')
...
#Last Behabior
genes <- c("NPTX2","MUC6","NRAP","AL157935.3","WFDC2","POTEC","SCEL","CYP2D6","GLDN","NEFH","ST6GAL2","B3GNT7","ADAMTS4","THPO","CPA6","ST3GAL4","CLDN8")
valified_qcFtest <-topTags(qcFtest, n=length(rownames(qcFtest)))
valified_qcFtest <- valified_qcFtest$table[,c("logFC","FDR")]
valified_qcFtest <- valified_qcFtest[which(rownames(valified_qcFtest) %in% genes),]
write.table(valified_qcFtest,paste0(work_dir,"02.Table/",name,"_valified_qcFtest_table.txt"), quote=F, sep='\t')
...
y_uniq_dt.lcpm <- t(y_uniq_dt.lcpm)
cols <- list(pheno=c("NSNP"="blue", "others"="orange"))
