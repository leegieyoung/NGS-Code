...
train.X <- y_uniq.lcpm[,which(colnames(y_uniq.lcpm) %in% names(Total.sample))]
test.X <- y_uniq.lcpm[,which(colnames(y_uniq.lcpm) %in% names(Tru.sample))]
...
png(filename=paste0(work_dir,"01.Plot/sPLSDA_trian_final_performance_plot.png"),width=20, height=15, units="cm", res=200)
print({
