...
dev.off()
write.table(Ftest$table,paste0(work_dir,"02.Table/",name,"_Ftest_rmCD_INvsNO_table.txt"), quote=F, sep='\t')

#Verified gene in Total & Truseq(Common genes, rmCD_INvsNO_qcFtest)
Verified_Gene <- read.table("/data/keeyoung/gy_RNA/06.output/CD_in_Tru_After/02.Table/CD_in_Tru_After_qcFtest_rmINvsNO_table.txt",sep='\t')
print({paste0("Verified_Genes's demension is : ", dim(Verified_Gene))})
Verified_Gene <- rownames(Verified_Gene)
vg_Ftest <- Ftest[which(Ftest$genes$gene_name %in% Verified_Gene),]
print({paste0("Verified Ftest's gene's demension is :", dim(vg_Ftest))})

idfound <- vg_Ftest$gene$gene_name %in% mappedRkeys(org.Hs.egSYMBOL)
vg_Ftest <- vg_Ftest[idfound,]
egENTREZID <- toTable(org.Hs.egSYMBOL)
entrez_m <- match(vg_Ftest$genes$gene_name , egENTREZID$symbol)
vg_Ftest$genes$entrezID <- egENTREZID$gene_id[entrez_m]
David <- cbind(vg_Ftest$genes$entrezID ,vg_Ftest$table$logFC, vg_Ftest$gene$gene_name)
print({paste0("David's dimention is : ", dim(David))})
write.table(David,paste0(work_dir,"02.Table/",name,"_vg_Ftest_rmCD_INvsNO_table_David.txt"), quote=F, sep=',')

print("Pathway analyzed vg_Ftest")
GO_func(work_dir,Ftest)
KEGG_func(work_dir,Ftest)


#Genes that pass through fdr and logFC
w <- which(rownames(y_uniq) %in% rownames(vg_Ftest$genes))
y_uniq_dt <- y_uniq[w,]
