...
######WGCNA#####################
L_NR.pheno <- sample.pheno[which(sample.pheno %in% "L_NR")]
L_NR.frac <- Cell_frac[which(rownames(Cell_frac) %in% names(L_NR.pheno)),]
y_uniq <- y_uniq[,which(colnames(y_uniq) %in% names(L_NR.pheno))]
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_Scale independence.pdf"), width=10, height=6)
cex1 = 0.9;
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_Mean connectivity.pdf"), width=10, height=6)
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_Pre_dengrogram.pdf"), width=10, height=12)
...
# Plot the result
pdf(paste0(work_dir,"04.WGCNA/L_NR_Cluster_module_eigengenes.pdf"), width=5, height=6)
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_After_dengrogram.pdf"), width=10, height=12)
...
#L_NR.frac
moduleTraitCor = cor(MEs, L_NR.frac, use = "p");
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);
textMatrix = paste(signif(moduleTraitCor, 2), "\n(",signif(moduleTraitPvalue, 1), ")", sep = "")
dim(textMatrix) = dim(moduleTraitCor)

pdf(paste0(work_dir,"04.WGCNA/L_NR_L_NR.frac_Module-trait_relationships.pdf"), width=10, height=6)
labeledHeatmap(Matrix = moduleTraitCor,
    xLabels = names(L_NR.frac),
    yLabels = names(MEs),
    ySymbols = names(MEs),
    colorLabels = FALSE,
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_Network_heatmap_plot_all_genes.pdf"), width=10, height=8)
...
pdf(paste0(work_dir,"04.WGCNA/L_NR_Eigengene_adjacency_heatmap.pdf"), width=10, height=8)
