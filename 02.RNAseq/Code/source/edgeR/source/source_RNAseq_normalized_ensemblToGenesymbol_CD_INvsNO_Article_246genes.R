...
print({
    paste0("The valified_Ftest's demension is : ", dim(valified_Ftest))
    })  
write.table(valified_Ftest$table ,paste0(work_dir,"02.Table/",name,"_Ftest_table_246.txt"), quote=F, sep=',')
...
#246 Genes 
#w_vali <- which(rownames(y_uniq) %in% rownames(valified_Ftest$genes))
#w_vali <- which(rownames(y_uniq) %in% rownames(valified_Ftest[match("uniq",valified_Ftest$Expression),]))
...
write.table(y_uniq_vali.lcpm.scale, paste0(work_dir,"02.Table/246_Heatmap.csv"), quote=F, col.names=T, row.names=T,sep=",")
