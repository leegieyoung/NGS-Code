...
rownames(y_uniq.zscore) <- y_uniq$genes$gene_name
y_uniq.zscore <- t(scale(t(y_uniq.zscore)))
write.table(y_uniq.zscore, paste0(work_dir,"02.Table/y_uniq_noQC.zscore.txt"), quote=F, col.names=T, row.names=T,sep="\t")
...
y_uniq.lcpm <- cpm(y_uniq, log=TRUE)
write.table(y_uniq.cpm, paste0(work_dir,"02.Table","/",name,"_cpm_Norm.txt"), sep='\t', quote=F,col.names=T, row.names=T)
write.table(y_uniq.lcpm, paste0(work_dir,"02.Table","/",name,"_lcpm_Norm.txt"), sep='\t', quote=F,col.names=T, row.names=T)

y_uniq.rpkm <- rpkm(y_uniq)
y_uniq.lrpkm <- rpkm(y_uniq,log=TRUE)
write.table(y_uniq.rpkm, paste0(work_dir,"02.Table","/",name,"_rpkm_Norm.txt"), sep='\t', quote=F,col.names=T, row.names=T)
write.table(y_uniq.lrpkm, paste0(work_dir,"02.Table","/",name,"_lrpkm_Norm.txt"), sep='\t', quote=F,col.names=T, row.names=T)
