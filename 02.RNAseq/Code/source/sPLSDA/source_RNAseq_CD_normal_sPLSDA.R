...
tune.splsda.train <- tune.splsda(train.X, train.Y, ncomp = 3, # calculate for first 4 components
                                 validation = 'Mfold',
                                 folds = 10, nrepeat = 50, # use repeated cross-validation
...
print({plot(tune.splsda.train, col = color.jet(3))})
dev.off()
...
train.col <- gsub("stricture","#F68B33",train.col) #orange
train.col <- gsub("penetrating","#C2C2C2",train.col) #gray

legend=list(legend=c("NSNP","stricture","penetrating"),
