...
#Sample data
Total_CDin <- read.table("/data/keeyoung/gy_RNA/06.output/CD_in/02.Table/CD_in_Common_lcpm_Norm.txt", sep='\t')
Tru_CDin <- read.table("/data/keeyoung/gy_RNA/06.output/CD_in_Tru/02.Table/CD_in_Tru_Common_lcpm_Norm.txt", sep='\t')
#order
oTotal <- order(rownames(Total_CDin))
Total_CDin <- Total_CDin[oTotal,]
#Find NA
if(sum(is.na(rownames(Total_CDin))!=0)){
print("")
print("NA found")
print("")}

#order
oTru <- order(rownames(Tru_CDin))
Tru_CDin <- Tru_CDin[oTru,]
#Find NA
if(sum(is.na(rownames(Tru_CDin)))!=0){
print("")
print("NA found")
print("")}
...
train.X <- merge[,which(colnames(merge) %in% colnames(Total_CDin))]
test.X <- merge[,which(colnames(merge) %in% colnames(Tru_CDin))]

train.Y <- sample.group[which(names(sample.group) %in% colnames(Total_CDin))]
sample.meta$V4[match(names(train.Y), gsub("-",".",sample.meta$V1))]
test.Y <- sample.group[which(names(sample.group) %in% colnames(Tru_CDin))]
sample.meta$V4[match(names(test.Y), gsub("-",".",sample.meta$V1))]
...
tune.splsda.train <- tune.splsda(train.X, train.Y, ncomp = 5, # calculate for first 4 components
                                 validation = 'Mfold',
                                 folds = 10, nrepeat = 50, # use repeated cross-validation
...
#print({plot(tune.splsda.train, col = color.jet(5))})
print({plot(tune.splsda.train, col = color.jet(5),
	cex.axis=300)})
dev.off()
...
train.col <- train.Y
train.col <- gsub("^L_NR$","#388ECC",train.col) #blue
#train.col <- gsub("^L_NR$","#C2C2C2",train.col) #gray
train.col <- gsub("^L_R$","#F68B33",train.col) #orange

legend=list(legend=c("L_NR","L_R"),
			col=c("#388ECC","#F68B33"),
...
#
png(filename=paste0(work_dir,"01.Plot/sPLSDA_final_comp1.png"), width=60, height=50, units="cm", res=200)
#print({plotIndiv(y_uniq.final.splsda, comp = c(1,2),
print({plotIndiv(train.final.splsda, comp=c(1,1),
...
#Loading Plot
png(filename=paste0(work_dir,"01.Plot/sPLSDA_loading_Plot.png"),width=30, height=30, units="cm", res=200)
print({
plotLoadings(train.final.splsda, comp=1, method = 'median', contrib = 'max', size.name=0.7, ndisplay=20, title="Loading Plot comp1")
})
dev.off()
