...
#cp /ichrogene/project/temp/gylee/0.GWAS/1.Input/KNIH_72295ea_info0.8/1.QC/KNIH_72295ea_info0.8_pheno.txt ${Output_dir}/${Sample}_pheno.txt
...
${p2_Dir}/plink2 --pfile ${Output_dir}/imputation \
 ...
 --make-pgen \
 --memory ${MEM} \
 --update-sex /ichrogene/project/temp/gylee/0.GWAS/1.Input/KNIH_72295ea_info0.8/1.QC/KNIH_72295ea_info0.8.for_sex.txt \
...
singularity exec --bind /ichrogene/:/ichrogene/ /ichrogene/project/temp/gylee/Singularity/GWAS.sif Rscript /ichrogene/project/temp/gylee/0.GWAS/Code/R/Manhattan_plot.R

bash /ichrogene/project/temp/gylee/0.GWAS/Code/plink_code_folder/3_prune_KOR.sh ${Sample} ${Thread} ${MEM}

singularity exec --bind /ichrogene/:/ichrogene/ /ichrogene/project/temp/gylee/Singularity/GWAS.sif Rscript /ichrogene/project/temp/gylee/0.GWAS/Code/R/make_marker.R

run_if_file_exists() {
    local file="${Output_dir}/prune/${CUTOFF}.list"

    if [ -e "$file" ]; then
        echo "파일이 존재합니다. 파일 이름: $file"

#bash /ichrogene/project/temp/gylee/0.GWAS/Code/plink_code_folder/3_prune_customCUTOFF_KOR.sh ${Sample} ${Thread} ${MEM} ${CUTOFF}
