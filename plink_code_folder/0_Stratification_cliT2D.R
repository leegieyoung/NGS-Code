...
Sample="Ukb_cliT2D_GB"
INPUT_Dir=paste0(QC_Dir,Sample,"/1.QC/")
#
test <- read.table(paste0(INPUT_Dir,"sample/","cliT2D.txt"), head=T)

test$PHENO <- gsub(1,0,test$PHENO)
test$PHENO <- gsub(2,1,test$PHENO)
test$PHENO <- as.numeric(test$PHENO)
test$Gender <- as.numeric(test$Gender)

FID <- (sort(test$FID))
m <- match(FID, test$FID)
test <- test[m,]

#Only Gender
test_m <- matchit(PHENO ~ Gender, ratio=5, method="nearest", data=test)
print({test_m})
