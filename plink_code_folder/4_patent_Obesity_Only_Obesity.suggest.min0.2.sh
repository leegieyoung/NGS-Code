...
echo "----------------------------"
echo ""
#cat ${Output_dir}/prune/premium_173.T2D.txt ${Dir}/2.plink_result/Imputed_Ukb_obesity_BMI_WC_premium/prune/suggest.min0.2.prune.in | sort | uniq > ${Output_dir}/prune/Obesity.suggest.min0.2_Only_Obesity.txt
rm -rf ${Output_dir}/prune/raw.Obesity.suggest.min0.2_Only_Obesity.glm
touch ${Output_dir}/prune/raw.Obesity.suggest.min0.2_Only_Obesity.glm

for A in $(cat ${Dir}/2.plink_result/Imputed_Ukb_obesity_BMI_WC_premium/prune/suggest.min0.2.prune.in)
do
grep -w "${A}" ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid >> ${Output_dir}/prune/raw.Obesity.suggest.min0.2_Only_Obesity.glm
done

head -n 1 ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid > ${Output_dir}/header
cat ${Output_dir}/header ${Output_dir}/prune/raw.Obesity.suggest.min0.2_Only_Obesity.glm > ${Output_dir}/prune/Obesity.suggest.min0.2_Only_Obesity.glm
rm -rf ${Output_dir}/prune/raw.Obesity.suggest.min0.2_Only_Obesity.glm

export CUTOFF="suggest.min0.2_Only_Obesity"
singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/GWAS.sif Rscript /mnt/nas/gylee/0.GWAS/Code/R/make_marker_Obe_T2D.R

nohup bash ${PRS_dir}/calculate_PRS.suggest.min0.2_Only_Obesity.sh ${Sample} type2diabet obesity &
