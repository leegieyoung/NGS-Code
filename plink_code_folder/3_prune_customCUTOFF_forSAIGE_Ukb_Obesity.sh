...
do
grep -w "${A}" /mnt/nas/gylee/0.GWAS/2.SAIGE_result/Ukb_Train_Obesity/result/Ukb_Train_Obesity.result >> ${Output_dir}/prune/raw.${CUTOFF}.glm #SAIGE
done

head -n 1 /mnt/nas/gylee/0.GWAS/2.SAIGE_result/Ukb_Train_Obesity/result/Ukb_Train_Obesity.result > ${Output_dir}/header #SAIGE
...
##bash /mnt/nas/gylee/0.GWAS/4.PRS/KoGES/calculate_PRS.230802.KOR_forVali_cliT2D.sh ${Sample} T2D Obesity ${CUTOFF}
##bash /mnt/nas/gylee/0.GWAS/4.PRS/KoGES/calculate_PRS.230811.KOR_forTrain_cliT2D.MH.sh ${Sample} T2D Obesity ${CUTOFF}
