...
rm -rf ${Output_dir}/prune/raw.Obesity.suggest.min0.02.glm
touch ${Output_dir}/prune/raw.Obesity.suggest.min0.02.glm

for A in $(cat ${Output_dir}/prune/suggest.min0.02.prune.in)
do
grep -w "${A}" ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid >> ${Output_dir}/prune/raw.Obesity.suggest.min0.02.glm
done

head -n 1 ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid > ${Output_dir}/header
cat ${Output_dir}/header ${Output_dir}/prune/raw.Obesity.suggest.min0.02.glm > ${Output_dir}/prune/Obesity.suggest.min0.02.glm
rm -rf ${Output_dir}/prune/raw.Obesity.suggest.min0.02.glm

export CUTOFF="suggest.min0.02"
singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/GWAS.sif Rscript /mnt/nas/gylee/0.GWAS/Code/R/make_marker_Obe_T2D.R

nohup bash ${PRS_dir}/calculate_PRS.suggest.min0.02.sh ${Sample} type2diabet obesity &
...
#echo "----------------------------"
#echo ""
#cat ${Output_dir}/prune/premium_173.T2D.txt ${Dir}/2.plink_result/Imputed_Ukb_obesity_BMI_WC_premium/prune/1e-4.min0.02.prune.in | sort | uniq > ${Output_dir}/prune/Obesity.1e-4.min0.02.txt
#rm -rf ${Output_dir}/prune/raw.Obesity.1e-4.min0.02.glm
#touch ${Output_dir}/prune/raw.Obesity.1e-4.min0.02.glm
#
##for A in $(cat ${Output_dir}/prune/Obesity.1e-4.min0.02.txt)
##do
##grep -w "${A}" ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid >> ${Output_dir}/prune/raw.Obesity.1e-4.min0.02.glm
##done
#
#head -n 1 ${Output_dir}/${Sample}.PHENO1.glm.logistic.hybrid > ${Output_dir}/header
#cat ${Output_dir}/header ${Output_dir}/prune/raw.Obesity.1e-4.min0.02.glm > ${Output_dir}/prune/Obesity.1e-4.min0.02.glm
#rm -rf ${Output_dir}/prune/raw.Obesity.1e-4.min0.02.glm
#
#export CUTOFF="1e-4.min0.02"
#singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/GWAS.sif Rscript /mnt/nas/gylee/0.GWAS/Code/R/make_marker_Obe_T2D.R
#
#echo ""
#echo "----------------------------"
