...
make_pgen() {
 local file="/mnt/nas/gylee/0.GWAS/05.Imputation/2.minimac4/minimac4.1.0/${Sample}/${Sample}.IID"
 if [ -e "$file" ]; then
	 echo "파일이 존재합니다. 파일 이름: $file"
	 echo "make_pgen" >> ${Log}/logfile_${0}.txt

cp /mnt/nas/gylee/0.GWAS/05.Imputation/2.minimac4/minimac4.1.0/${Sample}/${Sample}_pheno.txt ${Output_dir}/${Sample}_pheno.txt
${p2_Dir}/plink2 --pfile /mnt/nas/gylee/0.GWAS/05.Imputation/2.minimac4/minimac4.1.0/Ukb_91584_R2_0.3/imputation \
...
make_indep() {
 local file="${Output_dir}/imputation_g_m_maf.pvar"
 if [ -e "$file" ]; then
	 echo "파일이 존재합니다. 파일 이름: $file"
...
#Make_COV
singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/GWAS.sif Rscript /mnt/nas/gylee/0.GWAS/Code/R/covariate_forSAIGE.R
...
singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/SAIGE/SAIGE_docker_R36.sif bash /mnt/nas/gylee/0.GWAS/Code/SAIGE_code_folder/SAIGE_totalGRM_CreateGRM_231016.sh ${Sample} ${Thread}
...
#SAIGE
do_SAIGE() {
 local file="${Dir}/2.SAIGE_result/${Sample}/sparseGRM/sparseGRM_relatednessCutoff_0.125_2000_randomMarkersUsed.sparseGRM.mtx"
if [ -e "$file" ]; then
	 echo "파일이 존재합니다. 파일 이름: $file"
...
for A in $(seq 1 22)
do
nohup singularity exec --bind /mnt/:/mnt/ /mnt/nas/gylee/Singurality/SAIGE/SAIGE_docker_R36.sif bash /mnt/nas/gylee/0.GWAS/Code/SAIGE_code_folder/SAIGE_totalGRM_231016.sh ${Sample} ${Thread} ${A} > ${Log}.chr${A}.doSAIGE.txt 2>&1 &
done
wait
