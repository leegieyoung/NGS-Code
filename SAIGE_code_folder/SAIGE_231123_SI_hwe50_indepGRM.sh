...
#Module
make_pgen() {
 local file="/ichrogene/project/temp/gylee/0.GWAS/05.Imputation/2.minimac4/minimac4.1.0/${Sample}/${Sample}.IID"
 if [ -e "$file" ]; then
...
bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/SAIGE_indepGRM_CreateGRM_231017.sh ${Sample} ${Thread}
...
for A in $(seq 1 20)
do
nohup bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/SAIGE_indepGRM2000_231017.sh ${Sample} ${Thread} ${A} > /dev/null 2>&1 &
sleep 2
done

for A in $(seq 21 22)
do
nohup bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/SAIGE_indepGRM1000_231017.sh ${Sample} ${Thread} ${A} > /dev/null 2>&1 &
sleep 2
done
wait
sleep 3
