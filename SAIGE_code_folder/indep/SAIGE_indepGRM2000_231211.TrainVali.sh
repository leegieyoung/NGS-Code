...
#     --nThreads=32  \
#     --outputPrefix=${SAIGE_dir}/sparseGRM/sparseGRM \
...
Rscript ${SAGE_CD}/step1_fitNULLGLMM.R     \
       --sparseGRMFile=${SAIGE_dir}/sparseGRM/chr${Chr}.sparseGRM_relatednessCutoff_0.125_2000_randomMarkersUsed.sparseGRM.mtx \
        --sparseGRMSampleIDFile=${SAIGE_dir}/sparseGRM/chr${Chr}.sparseGRM_relatednessCutoff_0.125_2000_randomMarkersUsed.sparseGRM.mtx.sampleIDs.txt \
        --plinkFile=${SAIGE_dir}/sparseGRM/LD_pruned_QC/chr${Chr}.LD_pruned_QC_${Sample}  \
	...
	--useSparseGRMforVarRatio=TRUE \
        ...
        --traitType=binary \
        --outputPrefix=${SAIGE_dir}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.indep \
...
Rscript ${SAGE_CD}/step2_SPAtests.R \
        ...
        --chrom=${Chr} \
        --GMMATmodelFile=${SAIGE_dir}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.indep.rda \
        --varianceRatioFile=${SAIGE_dir}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.indep.varianceRatio.txt \
        --LOCO=FALSE \
        --is_Firth_beta=TRUE \
	--is_imputed_data=TRUE \
        --pCutoffforFirth=0.125 \
        --is_output_moreDetails=TRUE \
        --SAIGEOutputFile=${SAIGE_dir}/result/chr${Chr}.${Sample}.LDpruned.indep.SAIGE.test.markers.txt \
        --is_fastTest=TRUE \
