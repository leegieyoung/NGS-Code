...
${p2_Dir}/plink2 --pfile /ichrogene/project/temp/gylee/0.GWAS/05.Imputation/2.minimac4/minimac4.1.0/Ukb_21.1_R2_0.3_MAF0.005/imputation \
...
bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/indep/SAIGE_indepGRM_CreateGRM_231211.TrainVali.sh ${Sample} ${Thread} ${TrainVali}
...
#SAIGE
do_SAIGE() {
 local file="${SAIGE_dir}/sparseGRM/chr1.sparseGRM_relatednessCutoff_0.125_2000_randomMarkersUsed.sparseGRM.mtx"
if [ -e "$file" ]; then
	 echo "파일이 존재합니다. 파일 이름: $file"
...
for A in $(seq 1 20)
do
 nohup bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/indep/SAIGE_indepGRM2000_231211_Specific_SEX.TrainVali.sh ${Sample} ${Thread} ${A} ${TrainVali} > ${Log}/do_SAIGE.chr${A}.indep2000.txt 2>&1 &
 sleep 2
done
wait
...
for A in $(seq 21 22)
do
 nohup bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/indep/SAIGE_indepGRM1000_231211_Specific_SEX.TrainVali.sh ${Sample} ${Thread} ${A} ${TrainVali} > ${Log}/do_SAIGE.chr${A}.indep1000.txt 2>&1 &
 sleep 2
done
wait
...
sleep 2
wait
bash /ichrogene/project/temp/gylee/0.GWAS/Code/SAIGE_code_folder/indep/Step3.merge.TrainVali.sh ${Sample} ${Thread} ${TrainVali} &
