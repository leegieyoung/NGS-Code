...
Rscript ${SAGE_CD}/step1_fitNULLGLMM.R     \
	...
	--useSparseGRMforVarRatio=TRUE \
        ...
        --traitType=binary \
        --outputPrefix=/mnt/nas/gylee/0.GWAS/2.SAIGE_result/${Sample}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.total_grmVarRatio \
        --IsOverwriteVarianceRatioFile=TRUE > ${Log}/chr${Chr}.${Sample}.LDpruned.total_grmVarRatio.log 2>&1
...
Rscript ${SAGE_CD}/step2_SPAtests.R \
        ...
        --chrom=${Chr} \
        --GMMATmodelFile=/mnt/nas/gylee/0.GWAS/2.SAIGE_result/${Sample}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.total_grmVarRatio.rda \
        --varianceRatioFile=/mnt/nas/gylee/0.GWAS/2.SAIGE_result/${Sample}/sparseGRM/fitNGLMM.chr${Chr}.${Sample}.LDpruned.total_grmVarRatio.varianceRatio.txt \
        --LOCO=FALSE \
        --is_Firth_beta=TRUE \
	--is_imputed_data=TRUE \
        --pCutoffforFirth=0.125 \
        --is_output_moreDetails=TRUE \
        --SAIGEOutputFile=${Dir}/2.SAIGE_result/${Sample}/result/chr${Chr}.${Sample}.LDpruned.total.SAIGE.test_grmVarRatio.markers.txt \
        --is_fastTest=TRUE \
        --is_output_moreDetails=TRUE > ${Log}/chr${Chr}.${Sample}.LDpruned.total.SAIGE.test_grmVarRatio.markers.log 2>&1
