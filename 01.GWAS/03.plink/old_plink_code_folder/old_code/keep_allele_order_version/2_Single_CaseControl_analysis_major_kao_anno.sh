#!/bin/sh
if [ $# -ne 2 ];then
        echo "Please enter Sample_Name and cutoff"
               exit
fi
Sample=$1
cutoff=$2
...
awk '$12 < 0.0005 {print $2}' ${analysis_folder}/merge/logistic/${Sample}_NoNA_assoc.assoc.logistic > ${analysis_folder}/merge/logistic/anno/${cutoff}_snp.list
plink --bfile ${analysis_folder}/merge/${Sample}_NoNA \
 --extract ${analysis_folder}/merge/logistic/anno/${cutoff}_snp.list \
 --recode vcf-iid \
 --double-id \
 --keep-allele-order \
 --out ${analysis_folder}/merge/logistic/anno/raw_${cutoff}_snp_vcf

awk '{print $1, $2, $3, $4, $5, $6, $7, $8}' ${analysis_folder}/merge/logistic/anno/raw_${cutoff}_snp_vcf.vcf >  ${analysis_folder}/merge/logistic/anno/raw_${cutoff}_snp_vcf_rmSam.vcf
grep '^#' ${analysis_folder}/merge/logistic/anno/raw_${cutoff}_snp_vcf_rmSam.vcf > ${analysis_folder}/merge/logistic/anno/head.vcf
grep -v '^#' ${analysis_folder}/merge/logistic/anno/raw_${cutoff}_snp_vcf_rmSam.vcf > ${analysis_folder}/merge/logistic/anno/nohead.vcf
...
#snpsift dbsnp154
java -Xmx64g -jar /scratch/x1997a11/SnpEff/snpEff/SnpSift.jar annotate /scratch/x1997a11/REFERENCE/Anno/dbsnp154/dbsnp154.vcf.gz ${analysis_folder}/merge/logistic/anno/rmsnp.vcf > ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.vcf
...
plink --bfile ${analysis_folder}/merge/${Sample}_NoNA \
 ...
 --ci 0.95 \
 --beta \
 --out ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_assoc

plink --bfile ${analysis_folder}/merge/${Sample}_NoNA \
 --extract ${analysis_folder}/merge/logistic/anno/${cutoff}_snp.list \
 --freq case-control \
 --keep-allele-order \
 --out ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_freq

#summary_file
#rsID
grep -v '^#' ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.vcf > ${analysis_folder}/merge/logistic/anno/nohead_${cutoff}${Sample}_NoNA.vcf
awk '{print $3}' ${analysis_folder}/merge/logistic/anno/nohead_${cutoff}${Sample}_NoNA.vcf > ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.rsID
awk '{print $1, $3}' ${analysis_folder}/merge/logistic/anno/nohead_${cutoff}${Sample}_NoNA.vcf >  ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.chr_rsID
sed -i '1i\ rsID' ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.rsID

#freq

#pval
awk '{print $12}' ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_assoc.assoc.logistic > ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_assoc.pval

paste ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.rsID ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_freq.frq.cc ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_assoc.pval > ${analysis_folder}/merge/logistic/anno/summary_result.csv

#S-predixcan
mkdir ${analysis_folder}/merge/logistic/predixcan
awk '{print $7 ,$12}' ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_assoc.assoc.logistic > ${analysis_folder}/merge/logistic/predixcan/${cutoff}_${Sample}_NoNA.beta_pval

awk '{print $1}'  ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_freq.frq.cc > ${analysis_folder}/merge/logistic/predixcan/${cutoff}chr.txt
awk '{print $3, $4}'  ${analysis_folder}/merge/logistic/anno/${cutoff}_${Sample}_NoNA_freq.frq.cc > ${analysis_folder}/merge/logistic/predixcan/${cutoff}A1_A2.txt

paste ${analysis_folder}/merge/logistic/predixcan/${cutoff}chr.txt ${analysis_folder}/merge/logistic/anno/${cutoff}${Sample}_NoNA.rsID ${analysis_folder}/merge/logistic/predixcan/${cutoff}A1_A2.txt ${analysis_folder}/merge/logistic/predixcan/${cutoff}_${Sample}_NoNA.beta_pval > ${analysis_folder}/merge/logistic/predixcan/${cutoff}_${Sample}.predixcan

for A in $(seq 1 22)
do
grep -w "^${A}" ${analysis_folder}/merge/logistic/predixcan/${cutoff}_${Sample}.predixcan > ${analysis_folder}/merge/logistic/predixcan/raw_chr${A}_${Sample}.predixcan
