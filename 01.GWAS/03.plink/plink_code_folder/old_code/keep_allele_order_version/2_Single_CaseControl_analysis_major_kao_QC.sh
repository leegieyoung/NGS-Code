...
#=========What is Code ? =======================================
echo "${code_path}/2_Single_CaseControl_analysis_major_kao_noanno.sh" > ${analysis_folder}/merge/Used_code.md

plink --bfile ${QC_folder}/MaMi/QC_${Sample} \
 --geno 0.2 \
 --keep-allele-order \
 --make-bed \
 --out ${analysis_folder}/merge/NoQC_${Sample}_g

plink --bfile ${analysis_folder}/merge/NoQC_${Sample}_g \
 --mind 0.2 \
 --keep-allele-order \
 --make-bed \
 --out ${analysis_folder}/merge/NoQC_${Sample}_g_m

plink --bfile ${analysis_folder}/merge/NoQC_${Sample}_g_m \
 --maf 0.01 \
 --keep-allele-order \
 --make-bed \
 --out ${analysis_folder}/merge/NoQC_${Sample}_g_m_maf

plink --bfile ${analysis_folder}/merge/NoQC_${Sample}_g_m_maf \
 --hwe 1e-6 \
 --keep-allele-order \
 --make-bed \
 --out ${analysis_folder}/merge/NoQC_${Sample}_g_m_maf_hwe


plink --bfile ${analysis_folder}/merge/NoQC_${Sample}_g_m_maf_hwe \
 --keep-allele-order \
 --assoc \
