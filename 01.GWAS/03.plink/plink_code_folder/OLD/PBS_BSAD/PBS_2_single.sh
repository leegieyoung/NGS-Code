#!/bin/sh
#PBS -V
#PBS -q normal
#PBS -A etc
#PBS -l select=1:ncpus=68:mpiprocs=1:ompthreads=68
#PBS -l walltime=48:00:00
#PBS -o /scratch/x1997a11/PBS.OU
#PBS -e /scratch/x1997a11/Error
#PBS -N BSAD199_2_single

Code_path="/scratch/x1997a11/GWAS/pdxen_AD/Code_folder/keep_allele_order_version"
module purge
module load python/3.7
. /apps/applications/PYTHON/3.7/etc/profile.d/conda.sh
conda activate plink

sh ${Code_path}/2_Single_CaseControl_analysis_major_kao_anno_edit2.sh Imputed_AD_199
sh /scratch/x1997a11/GWAS/pdxen_AD/Code_folder/keep_allele_order_version/PBS_BSAD/repeat.sh
